...
%%%% === 参数扫描设置 ===
mu_vals      = -22 : -1 : -35;      % [-22, -23, ..., -35] 共 14 个
TotalC_vals  = 4200:100:6000;       % [4200, 4300, ..., 6000] 共 19 个
TotalC_ref   = 5000;                % 5000 对应原来 52e12/139e12 的标度
    ...
    for iF = 1:numel(TotalC_vals)
        ...
        for ii = 1: num_runs
            try
                ...
                pars.d13C_input = [-25, ...
                    truncnorm(mu, sigma, mu-4, mu+4), ...
                    truncnorm(mu, sigma, mu-4, mu+4), ...
                    truncnorm(mu, sigma, mu-4, mu+4)];

                pars.time       = [-3e6 0 100  6000];

                % 总碳输入按 TotalC 线性缩放（5000 → 52e12 / 139e12）
                pars.input_C    = [ 0  0 52e12 139e12] * (0.75+0.5*rand(1)) * scaleC;

                pars.k_ccdeg = 21e12 * (0.75+0.5*rand(1)) ; 
                ...
                pars.k_CH4_diff          = 0.8 + 0.15 * rand(1);          
        ...
        interpolated_results = cell(num_runs_eff, 1);
        ...
        d13c_DIC            = NaN(num_runs_eff, length(base_time));
        T_p                 = NaN(num_runs_eff, length(base_time));
        Atmospheric_CO2_ppm = NaN(num_runs_eff, length(base_time));
        Atmospheric_CH4_a_ppm = NaN(num_runs_eff, length(base_time));
        f_volcanic_CO2      = NaN(num_runs_eff, length(base_time));
        f_CH4               = NaN(num_runs_eff, length(base_time));
        d13C_input          = NaN(num_runs_eff, length(base_time));
        ...
        if imu == 1 && iF == 1
            ...
            fill([plot_time', fliplr(plot_time')],[d13c_DIC_5, fliplr(d13c_DIC_95)],color_line,'LineStyle','none','facealpha',0.2);
            fill([d13C_p(:,1)', fliplr(d13C_p(:,1)')],[d13C_p(:,2)', fliplr(d13C_p(:,4)')],color_scatter,'LineStyle','none','facealpha',0.2);
            ...
            fill([plot_time', fliplr(plot_time')],[Atmospheric_CO2_5, fliplr(Atmospheric_CO2_95)],color_line,'LineStyle','none','facealpha',0.2);
            fill([pco2(:,1)', fliplr(pco2(:,1)')],[pco2(:,2)', fliplr(pco2(:,4)')],color_scatter,'LineStyle','none','facealpha',0.2);
            ...
            fill([plot_time', fliplr(plot_time')],[T_p_5-273, fliplr(T_p_95-273)],color_line,'LineStyle','none','facealpha',0.2);
            fill([SST(:,1)', fliplr(SST(:,1)')],[SST(:,2)', fliplr(SST(:,4)')],color_scatter,'LineStyle','none','facealpha',0.2);
            ...
            fill([plot_time', fliplr(plot_time')],[f_CH4_5, fliplr(f_CH4_95)],color_line,'LineStyle','none','facealpha',0.2);
            d2=plot(plot_time, f_volcanic_CO2_mean,'-','LineWidth',2,'Color',color_shade);
            fill([plot_time', fliplr(plot_time')],[f_volcanic_CO2_5, fliplr(f_volcanic_CO2_95)],color_shade,'LineStyle','none','facealpha',0.2);
            xlim([0 6000])
            ...
            fill([plot_time', fliplr(plot_time')],[d13C_input_5, fliplr(d13C_input_95)],color_line,'LineStyle','none','facealpha',0.2);
            xlim([0 6000])
            xlabel('Time (yr)', 'FontSize', 12)
            ...
            fill([plot_time', fliplr(plot_time')],[Atmospheric_CH4_5, fliplr(Atmospheric_CH4_95)],color_line,'LineStyle','none','facealpha',0.2);
            xlim([0 6000])
            xlabel('Time (yr)', 'FontSize', 12)
