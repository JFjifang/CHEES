...
% 1. 定义时间序列 (包含 spin-up 阶段的 -3e6 和 0，以及后续每隔 100 年的数据点)
pars.time = [ ...
...
% % === Modified Isotope Input (Auto-Calculated) ===
% mu_mixed  = -28.1033; % 计算所得前段值
% mu_volc   = -26.0;   % 后段纯火山值
% spin_val  = -25;    % Spin-up 值
% 
% % 自动构建向量 (总长度 64)
% pars.d13C_input = [spin_val, spin_val, ...
%                    ones(1, 55) * mu_mixed, ...
%                    ones(1, 7) * mu_volc];
...
pars.d13c_DIC_h_0  = 2.3 ;
pars.d13c_DIC_d_0  = 2.3 ;
...
% === 修改后 ===
% 1. 减小 maxstep 到 200-500 年，防止大步长跳过剧烈变化点
% 2. 开启 'BDF', 'on' 增加稳定性
% 3. 显式设置 RelTol 和 AbsTol (防止默认值过严或过宽导致震荡)
options = odeset('maxstep', 500, ... 
                 'RelTol', 1e-3, ...
                 'AbsTol', 1e-6, ...
                 'BDF', 'on', ...
                 'NonNegative', [1:11 18:27 33:59]);
...
% === 新增代码开始：检查是否算到了终点 ===
% 如果结束时间远小于设定的 pars.whenend (允许 1 年的误差)，则认为模拟失败
if rawoutput.T(end) < (pars.whenend - 1)
    error('ODE Solver failed to reach end time. Stopped at t = %f', rawoutput.T(end));
end
% === 新增代码结束 ===
%%%%% start time counter
% tic
...
for i = 1:num_runs
    ...
    % === 修改开始：提取各库的 DIC 总量 (Moles) 作为权重 ===
    M_p = interpolated_results{i}.DIC_p;
    M_s = interpolated_results{i}.DIC_s;
    M_h = interpolated_results{i}.DIC_h;
    
    % 提取各库的同位素值
    D_p = interpolated_results{i}.d13c_DIC_p;
    D_s = interpolated_results{i}.d13c_DIC_s;
    D_h = interpolated_results{i}.d13c_DIC_h;
    
    % 计算加权平均： (Mass * Value) / Total_Mass
    % 注意：这里用点乘 (.*) 和点除 (./) 以防数据是向量
